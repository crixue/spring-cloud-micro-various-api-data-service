...
import com.xrj.springcloud.variousapidata.service.CityRemoteClient;
...
public class WeatherDataScheduleTask {
	...
	@Autowired
	private CityRemoteClient cityRemoteClient;

	public void syncWeatherData() {
		try {
			logger.info("[WeatherDataScheduleTask] 定时获取各个城市天气数据 - [start]");
			ServerResponse<List<City>> citiesRes = cityRemoteClient.getCities();
			List<City> cityList = citiesRes.getData();	
